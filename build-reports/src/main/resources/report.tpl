...
var thumbnailMe=function(element){var oh=element.clientHeight; var ow=element.clientWidth;var sh=75;var sw=75;element.style.height=sh+'px';element.style.width=sw+'px'; element.size='thumb';element.addEventListener('click', evt => { let el=evt.target;var start=null;var grow=function(time){if (start==null){start=time;}let prog=(time-start)/transitionTime;let nh=Math.min(oh,(oh-sh)*prog+sh);let nw=Math.min(ow,(ow-sw)*prog+sw);el.style.height=nh+'px';el.style.width=nw+'px';if (nh<oh || nw<ow){requestAnimationFrame(grow);}else{el.size='orig';}};var shrink=function(time){if (start==null){start=time;}let prog=(time-start)/transitionTime;let nh=Math.max(sh,oh-(oh-sh)*prog);let nw=Math.max(sw,ow-(ow-sw)*prog);el.style.height=nh+'px';el.style.width=nw+'px';if (nh > sh || nw > sw){requestAnimationFrame(shrink);}else{el.size = 'thumb';}};if (el.size === 'thumb'){requestAnimationFrame(grow);}else{requestAnimationFrame(shrink);}});};
